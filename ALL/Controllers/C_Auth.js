...
const authEmployee = async (req, res) => {
  ...
  try {
    ...
    employee ? employee.password == password ? res.status(200).json({ msg: 'Successfully Login!' }) : res.status(400).json({ msg: 'Email/Password incorrect!' }) : res.status(404).json({ msg: 'Email/Password incorrect!' })
  } catch (error) {
